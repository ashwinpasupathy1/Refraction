...
======================
Import .pzfx (XML) files into Refraction.

Extracts data tables, group names, titles, and maps TableType
to Refraction chart types. Writes data to a temp .xlsx file.
...
# Prism TableType to Refraction chart type mapping
TABLE_TYPE_MAP = {
    "OneWay": "bar",
    "TwoWay": "grouped_bar",
    "XY": "scatter",
...
def _table_to_rows(table):
    ...
    Layout matches Refraction's expected Excel format.
    """
...
def import_pzfx(file_path):
    ...
    try:
        ...
        temp_dir = tempfile.mkdtemp(prefix="refraction_import_")
